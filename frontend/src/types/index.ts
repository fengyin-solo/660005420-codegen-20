...
export interface TimeWindow { start: number; end: number; count: number; parsed: number; levels: Record<string,number>; sources: Record<string,number>; firstTimestamp: string; lastTimestamp: string }
...
export interface AnalysisResult { logs: LogEntry[]; windows: TimeWindow[]; anomalies: AnomalyScore[]; alerts: Alert[]; totalLogs: number; totalParsed: number; sourceRoster: string[]; logType: string }
