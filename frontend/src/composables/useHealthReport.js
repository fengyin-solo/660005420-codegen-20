...
function persistPeriod() {
    try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify(period.value));
    }
    catch { /* 忽略写入失败 */ }
}
...
export function useHealthReport() {
    ...
    const metrics = computed(() => {
        ...
        if (roster.length === 0)
            present.forEach(s => onlineSources.push(s));
...
export function downloadBaseName(m) {
    const ts = new Date();
...
export function buildTextReport(m) {
    const lines = [];
...
export function buildJsonReport(m) {
    if (m.empty) {
